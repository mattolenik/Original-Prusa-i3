...
module assembly() {
    ...
    difference() {
        down(plate_h/2+base_thickness) left(plate_w/2+margin)
            foot(l, w, w_top, h, anchor=BOTTOM+LEFT+FWD);
        front_plate("holes", 50);
        yshift(w_top-1) front_plate("holes-large", 30);
        scale(1.001) front_plate("solid", plate_t);
    }
}

module foot(l, w, w_top, h, anchor, spin, orient) {
    fh = 10;
    w1=w+3;
    w2=w-2;
    fwd(plate_t/2) prismoid(size1=[l, w], size2=[l, w_top], shift=[0, -(w-w_top)/2], h=h, anchor=FRONT+BOTTOM+LEFT);
    cuboid([l+15, plate_t*3, fh], anchor=CENTER+BOTTOM+LEFT, chamfer=4.1, edges=[FRONT+TOP]);
